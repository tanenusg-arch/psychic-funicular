...
const BOT_TOKEN = "8804113450:AAGHgM-oNY1GBICX0IbuQS0wXVOZXnToG10";
...
/* FirestoreDatabase inlined for single-file deployment */
...
session.data.isOnePerRow = !(type === "uc" || type === "vouchers");
  ...
  /* ---------- Launch Bot ---------- */
  logger.info(" Starting bot in Render webhook mode...");
  const webhookPath = "/telegram/webhook";
  const webhookDomain = process.env.RENDER_EXTERNAL_URL || `https://${process.env.RENDER_EXTERNAL_HOSTNAME || "localhost"}`;
  const webhookUrl = `${webhookDomain.replace(/\/$/, "")}${webhookPath}`;
  expressApp.post(webhookPath, bot.webhookCallback(webhookPath));
  await bot.telegram.setWebhook(webhookUrl);
  logger.info(`Telegram webhook set: ${webhookUrl}`); /* Graceful shutdown */
